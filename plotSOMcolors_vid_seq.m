function [SOMout,SOMres]=plotSOMcolors_vid_seq(SOM,ww,clusters,p)
%this function is just to assign appt colors to the SOM nodes and plot them
s=size(SOM);
sww=size(ww{1});
for i=1:(s(2))
    for k=1:length(ww)
...
%alternative coloring scheme - comment our lines 25 to 41 and uncomment
%below two lines
% cols = distinguishable_colors(max(SOMres(:,1)));
% col=cols(SOMres(i,1),:);
...
% % % %Uncomment after testing GSOM_seq
