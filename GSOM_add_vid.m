...
function [SOM,SOMout]=GSOM_add_vid(SOM,ww,clusters,p)
h=figure;
movegui(h, 'onscreen');
title('test')
vidObj = VideoWriter(pwd);
vidObj.Quality = 100;
open(vidObj);
...
for j=1:nsom
    ...
    E=sum(errs);
    if (E-E_old)/length(errs)>0.1%Growth threshold
%         (E-E_old)/(1-cos_sim(a))
        %create new neuron
...
[SOMout,SOMres]=plotSOMcolors_vid(w,ww,clusters,p);
size(SOMout)
h=image(SOMout);
